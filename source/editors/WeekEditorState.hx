...
class WeekEditorState extends MusicBeatState
{
	...
	public function new(weekFile:WeekFile = null)
	{
		...
		else weekFileName = 'weekPresent1';
	}
	...
	public static var weekFileName:String = 'weekPresent1';
	...
	function addWeekUI() {
		...
		tab_group.add(new FlxText(10, backgroundInputText.y - 18, 0, 'Background Asset:'));
		tab_group.add(new FlxText(backgroundInputText.x, displayNameInputText.y - 18, 0, 'Display Name:'));
		tab_group.add(new FlxText(displayNameInputText.x, weekNameInputText.y - 18, 0, 'Week Name (for Reset Score Menu):'));
		tab_group.add(new FlxText(weekNameInputText.x, weekFileInputText.y - 18, 0, 'Week File:'));
	...
	private static function onLoadComplete(_):Void
	{
		...
		if(fullPath != null) {
			...
			if(rawJson != null) {
				...
				if(loadedWeek.songs != null && loadedWeek.weekName != null) //Make sure it's really a week
				{
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	override function beatHit():Void
	{
		...
		{
			if (icon.isAnimated)
				icon.playAnim(icon.getCharacter() + '-normal');
		}
	}
}
